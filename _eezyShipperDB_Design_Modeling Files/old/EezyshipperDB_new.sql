...
CREATE TABLE IF NOT EXISTS `eezyshipper`.`tns_consignment` (
  ...
  `created_at` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`tns_cons_id`, `consignment_id`, `consignment_status_id`),
  INDEX `pk_tns_consignment_consignment_idx` (`consignment_id` ASC) ,
  INDEX `pk_tns_consignment_route_from_country_idx` (`route_from` ASC) ,
  INDEX `pk_tns_consignment_route_to_country_idx` (`route_to` ASC) ,
  INDEX `fk_tns_consignment_consignment_status1_idx` (`consignment_status_id` ASC) ,
  CONSTRAINT `pk_tns_consignment_consignment_idx`
    FOREIGN KEY (`pk_tns_consignment_consignment_idx`)
    REFERENCES `eezyshipper`.`consignments` (`consignments_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `pk_tns_route_from_country_idx`
    FOREIGN KEY (`route_from`)
    REFERENCES `eezyshipper`.`countries` (`country_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `pk_tns_consignment_route_to_country_idx`
    FOREIGN KEY (`route_to`)
    REFERENCES `eezyshipper`.`countries` (`country_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_tns_consignment_consignment_status1_idx`
...
CREATE TABLE IF NOT EXISTS `eezyshipper`.`orders` (
  ...
  INDEX `fk_orders_tnsconsignment_idx` (`consignment_id` ASC) ,
  ...
  CONSTRAINT `fk_orders_supplier_charges1_idx`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_orders_tnsconsignment_idx`
